...
class Rating {
  ...
  protected function setConn() {
    try {
      ...
      self::$conn = new PDO("mysql:host=".DBHOST."; dbname=".DBNAME.";unix_socket=/opt/local/var/run/mysql55/mysqld.sock", DBUSER, DBPASS);
  ...
  public function getRating($items, $rate = '') {
    $rtgstdy = $this->rtgstdyDb(); // check the db, see whether user has already rated
  ...
  protected function setRtgDb($items, $rate, $rtgstdy) {
    ...
    //get course id by cmid/id
    $courseid = '';
    if (strpos($items[0],'_50_')) {
        // user is rating a course
        if ($result = $DB->get_record('course', array('id'=>substr($items[0], 6)), 'id', 'IGNORE_MISSING')) {
            $courseid = $result->id;
        }
    } else {
        $cmid = substr($items[0], 6);
        if ($result = $DB->get_record('course_modules', array('id'=>$cmid), 'id,course', 'IGNORE_MISSING')) {
    ...
    if (!empty($courseid)) {
        ...
        if ($result = $DB->get_record('block_littlestar_item', array('item'=>$items[0]),'id,item,totalrate,nrrates', 'IGNORE_MISSING')) {
            $result->totalrate += $rate;
            $result->nrrates += 1;
            $ratingflag = $DB->update_record('block_littlestar_item', $result);
        } else {
            $rate_item = new stdClass();
            $rate_item->item = $items[0];
            $rate_item->course = $courseid;
            $rate_item->totalrate = $rate;
            $rate_item->nrrates = 1;
            $ratingflag = $DB->insert_record('block_littlestar_item', $rate_item);
        }

        if(!$result = $DB->get_record('block_littlestar_user', array('user'=>$this->rater, 'item'=> $items[0]), '*' ,'IGNORE_MISSING')) {
            $user_rate = new stdClass();
            $user_rate->user = $this->rater;
            $user_rate->item = $items[0];
            $user_rate->ratedate = make_timestamp(date('Y'), date('m'), date('d'));
            $ratingflag = $DB->insert_record('block_littlestar_user', $user_rate);
        }

        if ($ratingflag) {
            require_once($CFG->dirroot.'/lib/datalib.php');
            add_to_log($courseid, 'blocks', 'littlestar rate', 'block_littlestar.php', 'rate item (rt_[contextlevel]_[cmid/courseid]): '.$items[0], '', $USER->id);
        }

        // add curent rated item to the others today, and save them as string ',' in cookie (till the end of day)
        $rtgstdy[] = $items[0];
        setcookie("ratings", implode(',', array_unique($rtgstdy)), strtotime('tomorrow'));
    } // if not empty course id
  }
  ...
  protected function getRtgDb($items, $rtgstdy, $setrated) {
    ...
    $sql = 'SELECT * from {block_littlestar_item} WHERE';
    $extrasql = ' item = ';
    $items = array_map('addSlhs', $items);
    if (count($items) > 1) {
        $extrasql .= implode(' AND item = ', $items);
    } else {
        $extrasql .= $items[0];
    }

    if ($results = $DB->get_records_sql($sql.$extrasql)) {
        foreach ($results as $result) {
            $rated = in_array($result->item, $rtgstdy) ? $setrated + 1 : $setrated;   // add 1 if the item was ranked by the user today
            $re[$result->item] = array($result->totalrate, $result->nrrates, $rated);
        }
    }
  ...
  protected function rtgstdyDb() {
    ...
    $sql = 'SELECT item FROM {block_littlestar_user} WHERE user = :rater';
    $params['rater'] = $this->rater;
