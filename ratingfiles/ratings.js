...
function rateStars(spn) {
  ...
  if(itemrated_rtg == '' && spn.parentNode.parentNode.parentNode) itemrated_rtg = spn.parentNode.parentNode.parentNode.innerHTML;       // store the rating of rated item
    ...
    if (navigator.userAgent.indexOf("Firefox")!=-1) star_sp.setAttribute('style', 'width:'+((i_sp+1)*22)+'px; background:url("'+ 'http://' + document.domain + '/moodle/blocks/littlestar/ratingfiles/star2.png")');
    else {
      star_sp.style.width = ((i_sp+1)*22)+'px';
      star_sp.style.background = 'url("'+ 'http://' + document.domain + '/moodle/blocks/littlestar/ratingfiles/star2.png")';
    }
  }
}
...
function rtgAjax(elm, ratev) {
  ...
  cerere_http.open("POST",  'http://'+ document.domain +'/moodle/blocks/littlestar/ratingfiles/ratings.php', true);			// crate the request
